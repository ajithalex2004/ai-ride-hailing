        ...
        <div className={`min-h-screen bg-[var(--t-bg)] text-white flex flex-col overflow-hidden transition-colors duration-1000 ${view === 'POLICE' ? 'border-t-4 border-blue-600' : ''}`}>
            ...
            <header className="h-20 border-b border-[var(--t-border)] flex items-center justify-between px-8 bg-[var(--t-surface)] backdrop-blur-md z-50">
                ...
                <div className="flex items-center gap-8">
                    ...
                    <StatItem label="TOTAL FLEET" value="1,240" color="text-[var(--t-cyan)]" />
                    <StatItem label="ACTIVE INCIDENTS" value={incidents.length} color={view === 'POLICE' ? 'text-blue-400' : 'text-[var(--t-orange)]'} />
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <main className="flex-1 relative bg-[#0a0a0c]">
                    ...
                    <div className="absolute top-6 left-6 p-4 glass-panel rounded-xl border border-[var(--t-border)] max-w-xs transition-opacity hover:opacity-20 pointer-events-none backdrop-blur-xl">
                        ...
                        <div className="flex justify-between text-[8px] font-bold text-[var(--t-text-muted)] uppercase tracking-tighter">
                            <span>Safe</span>
                            <span>Hazardous</span>
                        </div>
                    </div>
                ...
                <aside className="w-72 border-l border-[var(--t-border)] bg-[var(--t-surface)] p-6 hidden xl:block font-mono overflow-y-auto">
                    ...
                    <div className="space-y-2 text-[10px] text-white/40">
                        ...
                        <p><span className="text-[var(--t-cyan)] font-bold">[TRAF]</span> AI_REROUTING: ACTIVE_NODE_MARINA</p>
                        <div className="h-px bg-white/5 my-4"></div>
                        <p className="animate-pulse text-[var(--t-cyan)] font-bold text-[9px]">MONOLITH_SYNC_STABLE</p>
                        <p className="text-[8px] text-[var(--t-text-muted)] mt-2">Uptime: 99.99% • Latency: 42ms</p>
                    </div>
                </aside>
            </div>

            {/* Footer Branding */}
            <footer className="h-8 bg-black flex items-center px-8 border-t border-[var(--t-border)] relative z-50">
                <p className="text-[9px] text-[var(--t-text-muted)] uppercase tracking-[0.2em] font-medium">Strategic Road Management Engine • Powered by EXL Solutions • © 2026</p>
            </footer>
        </div>
    );
}
