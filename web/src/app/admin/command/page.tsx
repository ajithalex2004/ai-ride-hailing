        ...
        <div className="min-h-screen bg-[var(--t-bg)] text-white p-8 selection:bg-[var(--t-accent)]/30" style={{fontFamily:"var(--font-sans)"}}>
            ...
            <div className="grid grid-cols-12 gap-8">
                ...
                <div className="col-span-9 space-y-6">
                    <div className="bg-[var(--t-card)] border border-[var(--t-border)] rounded-[40px] h-[700px] relative overflow-hidden group shadow-2xl shadow-black">
                        {/* Map Render Mock */}
                        ...
                        <div className="absolute bottom-10 right-10">
                            <div className="bg-black/80 backdrop-blur-2xl border border-[var(--t-border)] p-8 rounded-3xl min-w-[300px]">
                ...
                <div className="col-span-3 space-y-6">
                    <div className="bg-[var(--t-card)] border border-[var(--t-border)] rounded-[40px] p-8 h-[900px] flex flex-col">
                        <h3 className="text-[12px] font-black uppercase text-[var(--t-text-muted)] mb-10 tracking-[0.4em]">SLA_SURVIVAL_WALL</h3>
...
function TenantTab({ active, label, icon, onClick }: any) {
    return (
        <button
            onClick={onClick}
            className={`flex items-center gap-3 px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${active ? 'bg-white text-black italic' : 'text-[var(--t-text-muted)] hover:bg-white/5'
                }`}>
            {icon}
            {label}
        </button>
...
function IncidentPin({ severity, label }: any) {
    return (
        <div className="flex flex-col items-center group cursor-pointer">
            <div className={`px-3 py-1 rounded-full text-[9px] font-black mb-2 opacity-0 group-hover:opacity-100 transition-all ${severity === 'CRITICAL' ? 'bg-red-500 text-white' : 'bg-[var(--t-orange)] text-black'
                }`}>{label}</div>
...
function ActionCard({ label, icon, color }: any) {
    return (
        <div className="bg-[var(--t-card)] border border-[var(--t-border)] p-8 rounded-[32px] flex flex-col items-center text-center group cursor-pointer hover:border-white/20 transition-all hover:bg-[var(--t-card-hover)]">
