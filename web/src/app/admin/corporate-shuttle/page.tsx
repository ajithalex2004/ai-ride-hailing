        ...
        <div className="min-h-screen bg-[var(--t-bg)] text-[var(--t-text)] p-6 selection:bg-[var(--t-accent)]/30 selection:text-[var(--t-text)]" style={{ fontFamily: "var(--font-sans)" }}>
            ...
            <div className="flex justify-between items-end mb-10">
                <div>
                    ...
                    <h1 className="text-5xl font-black tracking-tighter italic uppercase text-[var(--t-text)] drop-shadow-2xl" style={{fontFamily:"var(--font-heading)",letterSpacing:"-0.02em"}}>SHUTTLE_COMMAND_HQ</h1>
                </div>
            ...
            {/* Navigation Rails */}
            <div className="flex gap-8 mb-10 border-b border-[var(--t-border)] relative">
            ...
            <div className="grid grid-cols-12 gap-8">
                ...
                <div className="col-span-12 lg:col-span-8 space-y-8">
                    ...
                    {activeTab === 'COMMAND' && (
                        <div className="space-y-8">
                            {/* Live HUD */}
                            <div className="bg-[var(--t-card)] border border-[var(--t-border)] rounded-[24px] h-[550px] relative overflow-hidden group shadow-inner">
                                ...
                                <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-20">
                                    <path d="M 100 200 Q 400 300 800 200" fill="none" stroke="var(--t-accent)" strokeWidth="2" strokeDasharray="8 8" className="animate-[dash_20s_linear_infinite]" />
                                </svg>
                                ...
                                <div className="absolute bottom-10 left-10 right-10 flex justify-between items-end">
                                    <div className="bg-[var(--t-surface)] backdrop-blur-3xl border border-[var(--t-border)] p-6 rounded-[20px] flex items-center gap-6 shadow-2xl">
                                        <div className="flex items-center gap-6">
                                            <div className="p-5 bg-[var(--t-accent)]/10 rounded-2xl text-[var(--t-cyan)]">
                                                <Zap size={36} />
                                            </div>
                                            <div>
                                                <div className="text-[12px] font-black uppercase text-[var(--t-text)] tracking-widest mb-1 italic">Peak_Orchestrator_Active</div>
                                                <div className="text-xl font-black font-mono tracking-tighter text-[var(--t-cyan)]" style={{fontFamily:"var(--font-mono)"}}>32 ACTIVE SHUTTLES</div>
                                            </div>
                                        </div>
                                        <button className="px-10 py-5 bg-[var(--t-accent)] text-black text-[12px] font-black uppercase italic rounded-2xl hover:scale-105 transition-all shadow-xl shadow-[var(--t-accent)]/30">Optimize_All_Slots</button>
                                    </div>
                                </div>
                            </div>
                    ...
                    {activeTab === 'ENROLLMENT' && (
                        <div className="bg-[var(--t-card)] border border-[var(--t-border)] rounded-[24px] p-8 space-y-12">
                            <div className="flex justify-between items-center">
                                <h2 className="text-2xl font-black uppercase italic tracking-tighter" style={{fontFamily:"var(--font-heading)"}}>Self_Enrollment_Portal</h2>
                            ...
                            <button className="w-full py-8 bg-[var(--t-accent)] text-[var(--t-accent-contrast)] text-[14px] font-black uppercase italic rounded-2xl hover:scale-[1.01] transition-all shadow-2xl">Request_Transport_PASS</button>
                        </div>
                    )}
                </div>
                ...
                <div className="col-span-12 lg:col-span-4 space-y-8">
                    ...
                    <div className="bg-[var(--t-card)] border border-[var(--t-border)] rounded-[24px] p-6 relative overflow-hidden group">
                        <div className="absolute -top-8 -right-12 w-40 h-40 bg-[var(--t-accent)]/10 blur-[80px] group-hover:bg-[var(--t-accent)]/20 transition-all rounded-full" />
                        <div className="flex justify-between items-center mb-12">
                            <h3 className="text-[11px] font-black uppercase text-[var(--t-text-muted)] tracking-[0.4em]" style={{fontFamily:"var(--font-heading)"}}>PEAK_INTELLIGENCE</h3>
                        ...
                        <div className="space-y-12">
                            ...
                            <div className="p-6 bg-[var(--t-accent)]/5 border border-[var(--t-cyan)]/20 rounded-2xl">
                                <div className="flex items-center gap-4 mb-3">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-widest text-[var(--t-text)]">Capacity_Warning</span>
                                </div>
                    ...
                    <div className="bg-[var(--t-card)] border border-[var(--t-border)] rounded-[24px] p-6">
                        <div className="flex justify-between items-center mb-12">
                            <h3 className="text-[11px] font-black uppercase text-[var(--t-text-muted)] tracking-[0.4em]" style={{fontFamily:"var(--font-heading)"}}>SLA_PENALTY_COUNTER</h3>
                    ...
                    <div className="bg-[var(--t-accent)]/10 border border-[var(--t-cyan)]/20 p-6 rounded-[24px] text-center group cursor-pointer hover:bg-[var(--t-accent)]/15 transition-all">
                        ...
                        <p className="text-3xl font-black italic tracking-tighter text-[var(--t-text)] mb-6">$42,480.00</p>
...
function NavRail({ active, label, onClick }: any) {
    return (
        <button
            onClick={onClick}
            className={`pb-6 px-4 text-[11px] font-black uppercase tracking-[0.3em] transition-all relative ${active ? 'text-[var(--t-text)] italic' : 'text-white/20 hover:text-white/60'
                }`}>
            {label}
    ...
    return (
        <div className="flex bg-[var(--t-surface)] border border-[var(--t-border)] px-8 py-5 rounded-[16px] items-center gap-8 shadow-2xl">
            ...
            <div>
                ...
                <div className="text-[11px] font-black text-[var(--t-text)] italic tracking-tighter">{dept}</div>
            </div>
        </div>
    );
}
...
function BusNode({ lat, lng, id, label, pulse }: any) {
    return (
        <div className="absolute group cursor-pointer" style={{ top: lat, left: lng }}>
            ...
            <div className="absolute left-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all bg-[var(--t-surface)] backdrop-blur-2xl border border-[var(--t-border)] px-4 py-2 rounded-xl text-[9px] font-black tracking-widest whitespace-nowrap italic pointer-events-none">
                <div className="text-[var(--t-text)] mb-1">{id}</div>
...
function Badge({ label, color }: any) {
    return (
        <div className="px-6 py-3 bg-[var(--t-surface)] backdrop-blur-3xl border border-[var(--t-border)] rounded-2xl flex items-center gap-4">
...
function StatBox({ title, value, trend }: any) {
    return (
        <div className="bg-[var(--t-card)] border border-[var(--t-border)] p-6 rounded-[20px] group hover:border-[var(--t-border)] transition-all">
            <div className="text-[11px] font-black uppercase text-[var(--t-text-muted)] tracking-[0.4em] mb-6">{title}</div>
            <div className="flex items-baseline gap-4">
                <span className="text-5xl font-black italic tracking-tighter text-[var(--t-text)]">{value}</span>
...
function PenaltyItem({ label, amount, status }: any) {
    return (
        <div className="flex justify-between items-center bg-white/5 p-6 rounded-2xl hover:bg-white/[0.08] transition-all cursor-pointer">
            ...
            <div className="text-right">
                <div className="text-lg font-black font-mono italic text-[var(--t-orange)]" style={{fontFamily:"var(--font-mono)"}}>{amount}</div>
            </div>
        </div>
    );
}
