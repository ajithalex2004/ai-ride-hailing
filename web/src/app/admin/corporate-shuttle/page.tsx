...
export default function CorporateShuttleDashboard() {
    ...
    return (
        <div className="min-h-screen bg-[#050507] text-white p-8 font-sans selection:bg-neon-blue/30 selection:text-white">
            ...
            <div className="flex justify-between items-end mb-16">
                <div>
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-grey">Corporate_Logistics_OS_V2</span>
                    </div>
            ...
            {/* Navigation Rails */}
            <div className="flex gap-12 mb-16 border-b border-white/5 relative">
            ...
            <div className="grid grid-cols-12 gap-8">
                ...
                <div className="col-span-12 lg:col-span-8 space-y-8">
                    ...
                    {activeTab === 'COMMAND' && (
                        <div className="space-y-8">
                            {/* Live HUD */}
                            <div className="bg-white/[0.02] border border-white/5 rounded-[48px] h-[550px] relative overflow-hidden group shadow-inner">
                                ...
                                <div className="absolute top-10 left-10 flex gap-4">
                                    ...
                                    <Badge label="Peak: High Demand" color="bg-pulse-orange" />
                                </div>

                                <div className="absolute bottom-10 left-10 right-10 flex justify-between items-end">
                                    <div className="bg-black/90 backdrop-blur-3xl border border-white/10 p-10 rounded-[40px] flex items-center gap-10 shadow-2xl">
                                        <div className="flex items-center gap-6">
                                            <div className="p-5 bg-neon-blue/10 rounded-3xl text-neon-blue">
                                                <Zap size={36} />
                                            </div>
                                            <div>
                                                <div className="text-[12px] font-black uppercase text-white tracking-widest mb-1 italic">Peak_Orchestrator_Active</div>
                                                <div className="text-xl font-black font-mono tracking-tighter text-neon-blue">32 ACTIVE SHUTTLES</div>
                                            </div>
                                        </div>
                                        <button className="px-10 py-5 bg-neon-blue text-black text-[12px] font-black uppercase italic rounded-2xl hover:scale-105 transition-all shadow-xl shadow-neon-blue/30">Optimize_All_Slots</button>
                                    </div>
                                </div>
                            </div>
                    ...
                    {activeTab === 'ENROLLMENT' && (
                        <div className="bg-white/[0.02] border border-white/5 rounded-[48px] p-12 space-y-12">
                ...
                <div className="col-span-12 lg:col-span-4 space-y-8">
                    ...
                    <div className="bg-white/[0.02] border border-white/5 rounded-[48px] p-10 relative overflow-hidden group">
                        <div className="absolute -top-12 -right-12 w-40 h-40 bg-neon-blue/10 blur-[80px] group-hover:bg-neon-blue/20 transition-all rounded-full" />
                        <div className="flex justify-between items-center mb-12">
                            <h3 className="text-[11px] font-black uppercase text-muted-grey tracking-[0.4em]">PEAK_INTELLIGENCE</h3>
                        ...
                        <div className="space-y-12">
                            ...
                            <div className="p-6 bg-neon-blue/5 border border-neon-blue/20 rounded-3xl">
                                <div className="flex items-center gap-4 mb-3">
                                    <AlertTriangle size={16} className="text-neon-blue" />
                                    <span className="text-[10px] font-black uppercase tracking-widest text-white">Capacity_Warning</span>
                                </div>
                                <p className="text-[11px] font-bold text-muted-grey uppercase leading-relaxed tracking-wider">
                    ...
                    <div className="bg-white/[0.02] border border-white/5 rounded-[48px] p-10">
                        <div className="flex justify-between items-center mb-12">
                            <h3 className="text-[11px] font-black uppercase text-muted-grey tracking-[0.4em]">SLA_PENALTY_COUNTER</h3>
                            <Clock size={18} className="text-pulse-orange" />
                        </div>
                        <div className="space-y-8">
                    ...
                    <div className="bg-neon-blue/10 border border-neon-blue/20 p-10 rounded-[48px] text-center group cursor-pointer hover:bg-neon-blue/15 transition-all">
                        <CreditCard size={40} className="mx-auto text-neon-blue mb-6 group-hover:scale-110 transition-transform" />
                        ...
                        <div className="text-[9px] font-black text-muted-grey uppercase tracking-widest">Billing Period: MAR_2026</div>
                    </div>
    ...
    return (
        <div className="flex bg-white/[0.03] border border-white/10 px-8 py-5 rounded-[28px] items-center gap-8 shadow-2xl">
            <div className="flex items-center gap-4">
                <Building2 size={24} className="text-neon-blue" />
                <div className="text-sm font-black tracking-tighter uppercase italic">{name}</div>
            </div>
            <div className="w-[1px] h-8 bg-white/10" />
            <div>
                <div className="text-[9px] font-black uppercase text-muted-grey tracking-widest mb-0.5">Primary_Dept</div>
...
function BusNode({ lat, lng, id, label, pulse }: any) {
    return (
        <div className="absolute group cursor-pointer" style={{ top: lat, left: lng }}>
            {pulse && <div className="w-10 h-10 rounded-full bg-neon-blue/20 animate-ping absolute -inset-3" />}
            <div className="w-4 h-4 rounded-full border-2 border-white/40 bg-neon-blue relative z-10 shadow-[0_0_20px_rgba(0,163,255,1)] group-hover:scale-125 transition-transform" />
            <div className="absolute left-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all bg-black/90 backdrop-blur-2xl border border-white/10 px-4 py-2 rounded-xl text-[9px] font-black tracking-widest whitespace-nowrap italic pointer-events-none">
                <div className="text-white mb-1">{id}</div>
                <div className="text-muted-grey uppercase text-[7px]">{label}</div>
            </div>
        </div>
    );
}
...
function Badge({ label, color }: any) {
    return (
        <div className="px-6 py-3 bg-black/80 backdrop-blur-3xl border border-white/10 rounded-2xl flex items-center gap-4">
...
function StatBox({ title, value, trend }: any) {
    return (
        <div className="bg-white/[0.02] border border-white/5 p-10 rounded-[40px] group hover:border-white/10 transition-all">
            <div className="text-[11px] font-black uppercase text-muted-grey tracking-[0.4em] mb-6">{title}</div>
            <div className="flex items-baseline gap-4">
                <span className="text-5xl font-black italic tracking-tighter text-white">{value}</span>
                <span className={`text-[12px] font-black italic ${trend.startsWith('+') ? 'text-green-400' : 'text-neon-blue'}`}>{trend}</span>
            </div>
        </div>
    );
}
...
function EnrollmentField({ label, placeholder, value, disabled, type, options }: any) {
    return (
        <div className="space-y-4">
            <label className="text-[10px] font-black uppercase text-muted-grey tracking-widest pl-2">{label}</label>
            {type === 'select' ? (
                <select className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 text-sm font-bold focus:outline-none focus:border-neon-blue transition-all appearance-none cursor-pointer">
                ...
                <input
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 text-sm font-bold focus:outline-none focus:border-neon-blue transition-all disabled:opacity-40" />
            )}
        </div>
    );
}
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex items-baseline gap-4">
                ...
                <span className="text-[11px] font-black uppercase text-muted-grey italic tracking-widest">{sub}</span>
            </div>
        </div>
    );
}
    ...
    return (
        <div className="flex justify-between items-center bg-white/5 p-6 rounded-3xl hover:bg-white/[0.08] transition-all cursor-pointer">
            <div className="flex items-center gap-5">
                <div className={`p-3 rounded-2xl ${status === 'TRIGGERED' ? 'bg-pulse-orange/10 text-pulse-orange' : 'bg-green-400/10 text-green-400'}`}>
                    <AlertTriangle size={20} />
                </div>
                <div>
                    <div className="text-xs font-black uppercase italic tracking-tighter">{label}</div>
                    <div className="text-[9px] font-bold text-muted-grey uppercase tracking-widest">{status}</div>
                </div>
            </div>
            <div className="text-right">
                <div className="text-lg font-black font-mono italic text-pulse-orange">{amount}</div>
            </div>
        </div>
    );
}
