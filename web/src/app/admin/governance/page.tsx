...
export default function GovernanceDashboard() {
    ...
    return (
        <div className="min-h-screen bg-[#060608] text-white p-8 font-sans selection:bg-neon-blue/30 overflow-x-hidden">
            {/* City-Scale Header */}
            <div className="flex justify-between items-start mb-16 px-4">
                <div className="relative">
                    <div className="absolute -left-12 top-1/2 -translate-y-1/2 w-4 h-40 bg-neon-blue/20 blur-3xl rounded-full" />
                    <div className="flex items-center gap-3 mb-3">
                        <Globe size={16} className="text-neon-blue animate-pulse" />
                        <span className="text-[11px] font-black uppercase tracking-[0.4em] text-muted-grey">City_Authority_Governance</span>
                    </div>
                    <h1 className="text-6xl font-black tracking-tighter italic uppercase text-white drop-shadow-2xl">GLOBAL_SYSTEM_OS</h1>
                </div>

                <div className="flex bg-white/[0.03] border border-white/10 p-1.5 rounded-[24px] backdrop-blur-3xl shadow-2xl">
            ...
            <div className="grid grid-cols-12 gap-8 px-4 pb-20">
                ...
                {governanceMode !== 'INTELLIGENCE' && (
                    <>
                        ...
                        <div className="col-span-12 grid grid-cols-4 gap-8 mb-8">
                            ...
                            <HealthTile label="Emergency SLA" value="11.2m" sub="Avg Response City-wide" color="text-neon-blue" icon={<Zap size={24} />} />
                            <HealthTile label="System Uptime (HA)" value="99.998%" sub="Microservice Mesh" color="text-pulse-orange" icon={<Server size={24} />} />
                        ...
                        <div className="col-span-8 space-y-8">
                            <div className="bg-white/[0.02] border border-white/5 rounded-[48px] h-[600px] relative overflow-hidden group shadow-inner">
                                ...
                                <PulsePoint lat="30%" lng="40%" label="Emergency_Cluster" color="bg-neon-blue" active />
                                ...
                                <div className="absolute top-12 left-12 flex gap-4">
                                    ...
                                    <Badge label="Emergency: High Demand" color="bg-pulse-orange" />
                                </div>
                            </div>
                        ...
                        <div className="col-span-4 space-y-8">
                            <div className="bg-white/[0.02] border border-white/5 rounded-[40px] p-8">
                                <h3 className="text-[12px] font-black uppercase text-muted-grey mb-12 tracking-[0.4em]">SYSTEM_HETEROGENEITY</h3>
                                <div className="space-y-10">
                                    <DomainBar label="Transport (Revenue-Driven)" value={74} color="bg-green-400" detail="ERP Integrated" />
                                    <DomainBar label="Emergency (SLA-Driven)" value={88} color="bg-neon-blue" detail="HA Hardened" />
                                    <DomainBar label="Incident Recovery" value={62} color="bg-pulse-orange" detail="Clearance Optimized" />
                                </div>
                            </div>
                            <div className="bg-neon-blue/5 border border-neon-blue/20 p-10 rounded-[48px] relative overflow-hidden group hover:bg-neon-blue/10 transition-all">
                                <div className="flex items-center gap-5 mb-8">
                                    <div className="p-4 bg-neon-blue/10 rounded-[28px] text-neon-blue group-hover:scale-110 transition-transform">
                                        <Zap size={32} />
                                    </div>
                                    <div>
                                        <div className="text-[12px] font-black uppercase italic tracking-tighter text-white">AI_Dispatch_Oracle</div>
                                        <div className="text-[9px] font-bold text-muted-grey uppercase tracking-widest">Priority Matching: Active</div>
                                    </div>
                                </div>
                ...
                {governanceMode === 'GLOBAL' && (
                    <div className="col-span-12 space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-700">
                        <div className="bg-white/[0.02] border border-white/5 rounded-[48px] p-12">
                            <div className="flex justify-between items-center mb-12">
                                <div>
                                    <h2 className="text-3xl font-black uppercase italic tracking-tighter">GLOBAL_OS_MATRIX</h2>
                                    <div className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-grey">Unified Cross-Domain Sovereignty</div>
                                </div>
                            ...
                            <div className="grid grid-cols-4 gap-8">
                                ...
                                <DomainMatrixCard domain="Emergency" type="AMBULANCE_OS" status="CRITICAL_PEAK" load="91%" icon={<Zap size={20} />} color="text-pulse-orange" />
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-8">
                            <div className="bg-white/[0.02] border border-white/5 rounded-[40px] p-10 space-y-8">
                                <h3 className="text-[12px] font-black uppercase text-muted-grey tracking-[0.4em]">SHARED_CORE_UTILIZATION</h3>
                            ...
                            <div className="bg-white/[0.02] border border-white/5 rounded-[40px] p-10 h-[300px] flex items-center justify-center relative overflow-hidden">
                                <Activity size={120} className="text-neon-blue/10 absolute opacity-20" />
                                <div className="text-center relative z-10">
                                    <div className="text-5xl font-black italic text-white tracking-tighter mb-2">99.998%</div>
                                    <div className="text-[10px] font-black uppercase text-muted-grey tracking-widest">SYSTEM_HA_AVAILABILITY</div>
                                </div>
                            </div>
                        </div>
                    </div>
...
function TabButton({ active, label, onClick }: any) {
    return (
        <button
            onClick={onClick}
            className={`px-10 py-5 rounded-[20px] text-[11px] font-black uppercase tracking-widest transition-all ${active ? 'bg-white text-black italic shadow-2xl' : 'text-muted-grey hover:bg-white/5'
                }`}>
            {label}
        </button>
    );
...
function IntelligenceCard({ title, value, sub, icon }: any) {
    return (
        <div className="bg-white/[0.02] border border-white/5 p-10 rounded-[40px] group hover:border-white/10 transition-all cursor-pointer">
            <div className="flex justify-between items-center mb-10">
                <div className="text-[11px] font-black uppercase text-muted-grey tracking-[0.4em]">{title}</div>
                <div className="text-neon-blue group-hover:scale-125 transition-transform duration-500">{icon}</div>
            </div>
...
function AiModule({ title, status, detail, icon, color }: any) {
    return (
        <div className="flex justify-between items-center bg-white/[0.03] p-8 rounded-[32px] border border-white/5 hover:border-white/20 transition-all group overflow-hidden relative">
            ...
            <div className="flex items-center gap-6 relative z-10">
                <div className="p-4 bg-white/5 rounded-2xl text-muted-grey group-hover:text-white transition-colors">
                    {icon}
                </div>
                <div>
                    <div className="text-xs font-black uppercase italic tracking-tighter text-white mb-1">{title}</div>
                    <div className="text-[9px] font-bold text-muted-grey uppercase tracking-widest">{detail}</div>
                </div>
            </div>
            <div className={`text-[10px] font-black uppercase italic relative z-10 ${color || 'text-neon-blue'}`}>{status}</div>
        </div>
    );
}
...
function HealthTile({ label, value, sub, color, icon }: any) {
    return (
        <div className="bg-white/[0.02] border border-white/5 p-8 rounded-[40px] group hover:border-white/10 transition-all cursor-pointer">
            <div className={`mb-6 ${color} p-4 bg-white/5 w-fit rounded-2xl group-hover:scale-110 transition-transform`}>
                {icon}
            </div>
            <div className="text-[11px] font-black uppercase text-muted-grey tracking-[0.2em] mb-3">{label}</div>
...
function PulsePoint({ lat, lng, label, color, active, warning }: any) {
    return (
        <div className="absolute group cursor-pointer" style={{ top: lat, left: lng }}>
            ...
            <div className="absolute left-6 top-0 opacity-0 group-hover:opacity-100 transition-all px-3 py-1 bg-black/80 backdrop-blur-xl border border-white/10 rounded-lg text-[8px] font-black tracking-widest whitespace-nowrap italic">{label}</div>
        </div>
    );
}
...
function Badge({ label, color }: any) {
    return (
        <div className="px-5 py-2.5 bg-black/60 backdrop-blur-3xl border border-white/10 rounded-2xl flex items-center gap-3">
        ...
        <div className="space-y-4">
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <div className="text-[8px] font-bold text-muted-grey uppercase tracking-widest">{detail}</div>
                </div>
...
function DomainMatrixCard({ domain, type, status, load, icon, color }: any) {
    return (
        <div className="bg-white/[0.03] border border-white/5 p-8 rounded-[32px] hover:border-white/10 transition-all group relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-30 transition-opacity">
                {icon}
            </div>
            <div className="text-[10px] font-black uppercase text-muted-grey tracking-[0.2em] mb-4">{type}</div>
            <div className="text-2xl font-black italic text-white tracking-tighter mb-4">{domain}</div>
            <div className="flex justify-between items-center">
                <div className={`text-[9px] font-black uppercase ${color || 'text-neon-blue'}`}>{status}</div>
...
function CoreStat({ label, value }: any) {
    return (
        <div className="flex justify-between items-center pb-4 border-b border-white/5 last:border-0">
            <div className="text-[10px] font-black uppercase text-white/40 tracking-widest leading-relaxed italic">{label}</div>
            <div className="text-[11px] font-black uppercase text-neon-blue tracking-tighter italic">{value}</div>
        </div>
    );
}
...
function LogPanel({ title }: any) {
    return (
        <div className="bg-white/[0.02] border border-white/5 rounded-[40px] p-8">
            <div className="flex justify-between items-center mb-10">
                <h3 className="text-[10px] font-black uppercase text-muted-grey tracking-[0.3em]">{title}</h3>
...
function LogEntry({ time, actor, msg }: any) {
    return (
        <div className="flex gap-4 group">
            <span className="text-[9px] font-mono font-black text-white/20 italic pt-1 group-hover:text-neon-blue transition-colors">{time}</span>
            <div>
                <div className="text-[9px] font-black uppercase tracking-tighter text-white/60 mb-0.5">{actor}</div>
                <p className="text-[10px] font-bold text-muted-grey uppercase tracking-wider leading-relaxed">{msg}</p>
            </div>
        </div>
    );
}
