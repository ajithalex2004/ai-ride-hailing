...
export default function GovernanceDashboard() {
    ...
    return (
        <div className="min-h-screen bg-[var(--t-bg)] text-[var(--t-text)] p-6 selection:bg-[var(--t-accent)]/30 overflow-x-hidden" style={{ fontFamily: "var(--font-sans)" }}>
            {/* City-Scale Header */}
            <div className="flex flex-col gap-6 mb-12 px-4">
                ...
                <div className="relative">
                    ...
                    <h1 className="text-4xl md:text-5xl font-black tracking-tighter italic uppercase drop-shadow-2xl" style={{fontFamily:"var(--font-heading)",letterSpacing:"-0.02em"}} style={{ color: 'var(--t-text)', fontFamily: 'var(--font-heading)' }}>GLOBAL_SYSTEM_OS</h1>
                </div>
            ...
            <div className="grid grid-cols-12 gap-8 px-4 pb-20">
                ...
                {governanceMode !== 'INTELLIGENCE' && (
                    <>
                        ...
                        <div className="col-span-12 grid grid-cols-4 gap-8 mb-8">
                            ...
                            <HealthTile label="Active Assets" value="2.1k" sub="Ambulances / Tows / Rides" color="text-[var(--t-text)]" icon={<Box size={24} />} />
                        </div>

                        {/* Global Infrastructure Map HUD */}
                        <div className="col-span-8 space-y-8">
                            <div className="bg-[var(--t-card)] border border-[var(--t-border)] rounded-[24px] h-[600px] relative overflow-hidden group shadow-inner">
                        ...
                        <div className="col-span-4 space-y-8">
                            <div className="bg-[var(--t-card)] border border-[var(--t-border)] rounded-[20px] p-6">
                                <h3 className="text-[12px] font-black uppercase text-[var(--t-text-muted)] mb-12 tracking-[0.4em]" style={{fontFamily:"var(--font-heading)"}}>SYSTEM_HETEROGENEITY</h3>
                            ...
                            <div className="bg-[var(--t-accent)]/5 border border-[var(--t-cyan)]/20 p-6 rounded-[24px] relative overflow-hidden group hover:bg-[var(--t-accent)]/10 transition-all">
                                <div className="flex items-center gap-5 mb-8">
                                    <div className="p-4 bg-[var(--t-accent)]/10 rounded-[16px] text-[var(--t-cyan)] group-hover:scale-110 transition-transform">
                                        <Zap size={32} />
                                    </div>
                                    <div>
                                        <div className="text-[12px] font-black uppercase italic tracking-tighter text-[var(--t-text)]">AI_Dispatch_Oracle</div>
                ...
                {governanceMode === 'GLOBAL' && (
                    <div className="col-span-12 space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-700">
                        <div className="bg-[var(--t-card)] border border-[var(--t-border)] rounded-[24px] p-8">
                            <div className="flex justify-between items-center mb-12">
                                <div>
                                    <h2 className="text-3xl font-black uppercase italic tracking-tighter" style={{fontFamily:"var(--font-heading)"}}>GLOBAL_OS_MATRIX</h2>
                        ...
                        <div className="grid grid-cols-2 gap-8">
                            <div className="bg-[var(--t-card)] border border-[var(--t-border)] rounded-[20px] p-6 space-y-8">
                                <h3 className="text-[12px] font-black uppercase text-[var(--t-text-muted)] tracking-[0.4em]" style={{fontFamily:"var(--font-heading)"}}>SHARED_CORE_UTILIZATION</h3>
                            ...
                            <div className="bg-[var(--t-card)] border border-[var(--t-border)] rounded-[20px] p-6 h-[300px] flex items-center justify-center relative overflow-hidden">
                                <Activity size={120} className="text-[var(--t-cyan)]/10 absolute opacity-20" />
                                <div className="text-center relative z-10">
                                    <div className="text-5xl font-black italic text-[var(--t-text)] tracking-tighter mb-2">99.998%</div>
...
function TabButton({ active, label, onClick }: any) {
    return (
        <button
            onClick={onClick}
            className={`px-10 py-5 rounded-[20px] text-[11px] font-black uppercase tracking-widest transition-all ${active ? 'bg-[var(--t-accent)] text-[var(--t-accent-contrast)] italic shadow-2xl' : 'text-[var(--t-text-muted)] hover:bg-white/5'
                }`}>
            {label}
        </button>
    );
...
function IntelligenceCard({ title, value, sub, icon }: any) {
    return (
        <div className="bg-[var(--t-card)] border border-[var(--t-border)] p-6 rounded-[20px] group hover:border-[var(--t-border)] transition-all cursor-pointer">
            ...
            <div className="text-4xl font-black italic tracking-tighter text-[var(--t-text)] mb-3">{value}</div>
...
function AiModule({ title, status, detail, icon, color }: any) {
    return (
        <div className="flex justify-between items-center bg-[var(--t-surface)] p-6 rounded-[16px] border border-[var(--t-border)] hover:border-white/20 transition-all group overflow-hidden relative">
            <div className="absolute inset-0 bg-[var(--t-card)] translate-y-full group-hover:translate-y-0 transition-transform duration-500" />
            <div className="flex items-center gap-6 relative z-10">
                <div className="p-4 bg-white/5 rounded-2xl text-[var(--t-text-muted)] group-hover:text-[var(--t-text)] transition-colors">
                    {icon}
                </div>
                <div>
                    <div className="text-xs font-black uppercase italic tracking-tighter text-[var(--t-text)] mb-1">{title}</div>
...
function PulsePoint({ lat, lng, label, color, active, warning }: any) {
    return (
        <div className="absolute group cursor-pointer" style={{ top: lat, left: lng }}>
            ...
            <div className="absolute left-6 top-0 opacity-0 group-hover:opacity-100 transition-all px-3 py-1 bg-[var(--t-surface)] backdrop-blur-xl border border-[var(--t-border)] rounded-lg text-[8px] font-black tracking-widest whitespace-nowrap italic">{label}</div>
        </div>
    );
}
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-end">
                <div>
                    <div className="text-[11px] font-black uppercase tracking-tighter italic text-[var(--t-text)] mb-1">{label}</div>
                    <div className="text-[8px] font-bold text-[var(--t-text-muted)] uppercase tracking-widest">{detail}</div>
                </div>
                <div className="text-xl font-black font-mono text-white/40" style={{fontFamily:"var(--font-mono)"}}>{value}%</div>
            </div>
...
function DomainMatrixCard({ domain, type, status, load, icon, color }: any) {
    return (
        <div className="bg-[var(--t-surface)] border border-[var(--t-border)] p-6 rounded-[16px] hover:border-[var(--t-border)] transition-all group relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-30 transition-opacity">
                {icon}
            </div>
            <div className="text-[10px] font-black uppercase text-[var(--t-text-muted)] tracking-[0.2em] mb-4">{type}</div>
            <div className="text-2xl font-black italic text-[var(--t-text)] tracking-tighter mb-4">{domain}</div>
...
function LogEntry({ time, actor, msg }: any) {
    return (
        <div className="flex gap-4 group">
            <span className="text-[9px] font-mono font-black text-white/20 italic pt-1 group-hover:text-[var(--t-cyan)] transition-colors" style={{fontFamily:"var(--font-mono)"}}>{time}</span>
            <div>
